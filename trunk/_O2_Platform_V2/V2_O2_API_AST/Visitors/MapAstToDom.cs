...
//ICSharpCode.NRefactory.Visitors.CodeDOMOutputVisitor.cs file
...
using V2.O2.API.AST.ExtensionMethods;
using V2.O2.API.AST.ExtensionMethods.CSharp;
using ICSharpCode.NRefactory.Ast;
using ICSharpCode.NRefactory.Visitors;
using ICSharpCode.NRefactory;

namespace V2.O2.API.AST.Visitors // was ICSharpCode.NRefactory.Visitors
{
	public class MapAstToDom : AbstractAstVisitor
    {
        ...
        // internal properties used during AST parse

        private Stack<CodeNamespace> namespaceDeclarations { get; set; }
        private Stack<CodeTypeDeclaration> typeDeclarations { get; set; }
        private Stack<CodeStatementCollection> codeStack { get; set; }
        private Stack<Breakable> breakableStack { get; set; }

        private List<CodeVariableDeclarationStatement> variables { get; set; }
        private List<CodeParameterDeclarationExpression> parameters { get; set; }

        public MapAstToDom() // made fields into public properties
        {
            //TypesDOM = new List<CodeTypeDeclaration>();
            //MethodsDOM = new List<CodeMemberMethod>();            
            //public Properties
            CompilationUnitToNameSpaces = new Dictionary<CompilationUnit, List<CodeNamespace>>();
            MethodsAstToDom = new Dictionary<MethodDeclaration,CodeMemberMethod>();
            MethodsDomToAst = new Dictionary<CodeMemberMethod,MethodDeclaration>();
            TypesAstToDom = new Dictionary<TypeDeclaration, CodeTypeDeclaration>();
            TypesDomToAst = new Dictionary<CodeTypeDeclaration, TypeDeclaration>();
            
            //internal Properties
            namespaceDeclarations = new Stack<CodeNamespace>();
            typeDeclarations = new Stack<CodeTypeDeclaration>();            
            codeStack = new Stack<CodeStatementCollection>();
            breakableStack = new Stack<Breakable>();
            variables = new List<CodeVariableDeclarationStatement>();
     ...
     /*   public void loadCode(string code)
        ...
        {
            if (CompilationUnitToNameSpaces.ContainsKey(compilationUnit).isFalse())
                CompilationUnitToNameSpaces.Add(compilationUnit, new List<CodeNamespace>());
            CompilationUnitToNameSpaces[compilationUnit].Add(codeNamespace);
        }
        //DC
        public void mapMethod(MethodDeclaration methodDeclaration, CodeMemberMethod codeMemberMethod)
        {
            MethodsAstToDom.Add(methodDeclaration, codeMemberMethod);
            MethodsDomToAst.Add(codeMemberMethod, methodDeclaration);
        }
        //DC
        public void mapType(TypeDeclaration typeDeclaration, CodeTypeDeclaration codeTypeMethod)
        {
            TypesAstToDom.Add(typeDeclaration, codeTypeMethod);
            TypesDomToAst.Add(codeTypeMethod, typeDeclaration);
        }                
		...
		public override object VisitCompilationUnit(CompilationUnit compilationUnit, object data)
		{
            ...
            mapCompilationUnit(compilationUnit, globalNamespace);  // DC
            if (data == null)                                // DC
                data = compilationUnit;                      // DC
		...
		public override object VisitNamespaceDeclaration(NamespaceDeclaration namespaceDeclaration, object data)
		{
            ...
            if (data is CompilationUnit)                                            // DC
                mapCompilationUnit(data as CompilationUnit, currentNamespace);      // Dc
		...
		public override object VisitTypeDeclaration(TypeDeclaration typeDeclaration, object data)
		{
            ...
            mapType(typeDeclaration, codeTypeDeclaration); // DC store references to both objects
		...
		public override object VisitMethodDeclaration(MethodDeclaration methodDeclaration, object data)
		{
            ...
            mapMethod(methodDeclaration, memberMethod); // DC store references to both objects
