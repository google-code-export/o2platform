...
"
        ...
        public static void GetActiveStatementCmd(string args)
        {
            ...
            foreach (CorActiveFunction caf in afa)
            {
                ...
                WriteOutput(String.Format(CultureInfo.InvariantCulture, "{0}. Method: {1}, IL offset: {2}",
                                          new Object[] {frameNo, f.FullName, caf.ILoffset}));
                frameNo++;
            }
        }
...
"
        ...
        public static void ShellCommand(string args)
        {
            ...
            var psi = new ProcessStartInfo(cmdShell, "/c " + Environment.ExpandEnvironmentVariables(args));
            ...
            {
                var t1 = new Thread(new OutputReader(shell.StandardOutput).StartReading);
                t1.Start();

                var t2 = new Thread(new OutputReader(shell.StandardError).StartReading);
                t2.Start();
        ...
        #region Nested type: OutputReader
