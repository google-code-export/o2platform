...
//        	if (webBrowser != null)
        	...
        	/*htmlPages_TreeView.AfterSelect (sender,e) 
        		=> {
        			try
        			{
        				PublicDI.log.info("TAG :::  {0}", e.Node.Tag.GetType());
        				if (e.Node.Tag is IHtmlPage)
        				{        					
        					showPageDetails((IHtmlPage)e.Node.Tag);
        				}        				        			
        			}
        			catch(Exception ex)
        			{
        				PublicDI.log.ex(ex,"prob", true);
        			}
        			 };
        	*/
    	...
    	/* NOT WORKING
    	public void openWebPage(string urlToOpen)
    	{
    		groupBoxes[0].invokeOnThread(
    			()=> {
    					if (webBrowser!= null)
