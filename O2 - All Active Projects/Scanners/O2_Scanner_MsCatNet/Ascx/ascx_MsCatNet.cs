    ...
    public partial class ascx_MsCatNet : UserControl
    {
        ...
        public ascx_MsCatNet()
        {
            ...
            if (DesignMode == false)
            {
                ...
                string defaultDirectoryWithOzasmtConversions = Path.Combine(DI.config.O2TempDir,
        ...
        private bool processFolder(string folderPath, string folderName, bool recursiveSearch)
        {
            if (Directory.Exists(folderPath))
            {
                ...
                string workDirectory = Path.Combine(DI.config.O2TempDir, folderName);
        ...
        {
            processFolder(DI.config.CurrentExecutableDirectory, "O2 Assemblies", false);
        }
        ...
        {
             DI.log.debug("Opening selected ozasmt file in a new window: {0}", fileSelected);
        ...
        private void processScanTargetQueueItem()
        {
            if (scanTargetsQueue.Count > 0)
            {
                 DI.log.info("Using Scan Queue. There are {0} scans on the queue", scanTargetsQueue.Count);
