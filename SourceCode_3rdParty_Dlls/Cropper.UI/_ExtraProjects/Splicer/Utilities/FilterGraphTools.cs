...
namespace Splicer.Utilities
{
    ...
    public sealed class FilterGraphTools
    {
        ...
        /// <param name="useIntelligentConnect"></param>
        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
                                          ...
                                          bool useIntelligentConnect)
        {
            ...
            IPin destPin = FindPinByDirection(downFilter, PinDirection.Input);
        ...
        static IPin FindPinByDirection(IBaseFilter filter, PinDirection direction)
        {
          ...
          if (iEnum == null) throw new InvalidOperationException("iEnum is null");
          ...
          while (Marshal.ReadInt32(fetched) == 1)
          {
            ...
            if (pPin == null && pinDir == direction) break;
            
            if (pPin != null) Marshal.ReleaseComObject(pPin);
                                          ...
                                          bool useIntelligentConnect)
        {
            ...
            sourcePin = FindPinByDirection(upFilter, PinDirection.Output);

            if (sourcePin == null)
                throw new ArgumentException("The source filter has no output pin");

            destPin = FindPinByDirection(downFilter, PinDirection.Input);
