...
namespace DemoApp
{
    public class WatiNPHP : WatinScript
    {
        public WatiNPHP()
        {
            this.LanguageName = "PHP";
            this.settings.CodeLanguage = AppSettings.CodeLanguages.PHP;
        }
        ...
        public override void AddScriptLine(string Line)
        {
            ...
            if (sbCode.Length == 0)
            {
                sbCode.Append("$Interface=new COM('WatiN.COMInterface') or die('Cannot create IE object');"+Environment.NewLine);
