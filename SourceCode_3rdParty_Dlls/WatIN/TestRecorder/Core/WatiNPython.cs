...
namespace DemoApp
{
    public class WatiNPython : WatinScript
    {
        //http://docs.python.org/lib/minimal-example.html
        public WatiNPython()
        {
            this.LanguageName = "Python";
            this.settings.CodeLanguage = AppSettings.CodeLanguages.Python;
        }
        ...
        public override void AddScriptLine(string Line)
        {
            ...
            if (sbCode.Length == 0)
            {
                sbCode.Append("Interface = win32com.client.Dispatch(\"WatiN.COMInterface\")" + Environment.NewLine);
        ...
        public override string DetermineFindMethod(string BrowserName, IHTMLElement element)
        {
            ...
            else
            {
                switch (strFindMethod)
                {
                    ...
                    default: 
                        line += "Interface.FindByCustom(\"" + strFindMethod + "\",\"" + strValue; break;
                }
            }
        ...
        public override string DetermineFindMethod(string BrowserName, IHTMLElement element, CheckedListBox.CheckedItemCollection CheckedItems)
        {
            ...
            for (int i = 0; i < CheckedItems.Count; i++)
            {
                ...
                switch (arrItem[0].ToLower())
                {
                    ...
                    default: 
                        varprefix += " && Interface.FindByCustom(\"" + arrItem[0] + "\",\"" + arrItem[1] + "\")"; break;
                }
            }
        ...
        public override string CompileScript(string ScriptCode, Template TemplateObj, bool RunScript)
        {
            ...
            if (!File.Exists(TemplateObj.StartupApplication))
            {
                ...
                {
                    TemplateObj.StartupApplication = @"c:\python\python.exe";
                }
                frm.ShowResourceList(TemplateObj, FunctionAssemblies, true);
            }
            ...
            string Filename = System.IO.Path.ChangeExtension(ExecutableFilename, ".py");
